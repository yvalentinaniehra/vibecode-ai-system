# 🔧 Final Integration Steps - OAuth Backend Registration

## ✅ Completed

### Backend Services (Rust)
- ✅ `oauth_service.rs` - PKCE + AES-256-GCM encryption
- ✅ `google_api_service.rs` - User info + token refresh
- ✅ `oauth_server.rs` - Local HTTP callback server  
- ✅ `services/mod.rs` - All services exported
- ✅ Dependencies added to `Cargo.toml`
- ✅ Cargo build: **0 errors** (25 warnings for unused code)

### Frontend (TypeScript)
- ✅ `useOAuth.ts` hook created
- ✅ Dashboard.tsx updated with Google sign-in button
- ✅ Handler `handleGoogleSignIn` integrated

---

## 🚨 Remaining: Register OAuth Commands in `lib.rs`

### Location
Insert after line **1350** in `src-tauri/src/lib.rs` (after Account Commands section)
...
const GOOGLE_CLIENT_ID: &str = "YOUR_CLIENT_ID_HERE"; // TODO: Get from user
const GOOGLE_CLIENT_SECRET: &str = "YOUR_CLIENT_SECRET_HERE"; // TODO: Get from user
...
### Then Update invoke_handler

Find line with `.invoke_handler(tauri::generate_handler![` and add:
...
# Run app and test OAuth
npm run tauri dev
```

Click "Sign in with Google" → Browser opens → Authorize → App receives token → Account saved!
